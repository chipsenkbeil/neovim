...
function M:load(opts)
  ...
  table.insert(
    ...
    vim.api.nvim_create_autocmd({ 'ColorScheme' }, {
      callback = function()
        utils.query_bg_hex_str({ write = self.__writer.write_fast }):on_done(function(_, color)
...
function M:supported(on_supported)
  -- This does not work right now as CSI queries are not supported by
  -- neovim as part of TermResponse
  local promise = require('vim.ui.img.utils').query_term('\027[c', {
    ...
    local _, _, caps = string.find(seq, '\027[%?%d+;([^c]*)c')
    if caps and caps ~= '' then
      for _, cap in ipairs(vim.split(caps, ';')) do
        -- As documented, 4 indicates sixel support
...
function M:__convert_to_sixel(img, opts, on_convert)
  ...
  if self.__bg_color then
    ...
    local utils = require('vim.ui.img.utils')
    utils.query_bg_hex_str(opts):on_done(function(err, color)
...
function M:__redraw()
    ...
    for _, id in ipairs(self:__get_sorted_redraw_ids()) do
      ...
      self:__load_sixel_data(img, opts, function(err, data)
        if err then
          ...
          writer.write(utils.codes.move_cursor({ col = pos.x, row = pos.y }), data)
          mark_redraw_done()
        end
      end)
    end
  ...
  if not ok then
    err = err or 'sixel redraw unknown error'
